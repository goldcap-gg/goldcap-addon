...
function GC.BoardRows.Label(verdict, pending)
  ...
  elseif bucket == "GOLD" then
    -- One unit, same as SAFE's, but rounded UP: a figure that rounds down promises the buy for
    -- less gold than it takes.
    local copper = verdict.needsGold
    local amount = copper >= 10000 and ("%dg"):format(math.ceil(copper / 10000))
      or ("%ds"):format(math.ceil(copper / 100))
