...
describe("Book", function()
  ...
  describe("NextAskAbove", function()
    it("returns the cheapest stocked ask strictly above the price", function()
      local book = levels({ 100, 50 }, { 120, 0 }, { 150, 30 })
      assert.equal(150, GC.Book.NextAskAbove(book, 100))
      assert.equal(150, GC.Book.NextAskAbove(book, 149))
      assert.is_nil(GC.Book.NextAskAbove(book, 150))
      assert.is_nil(GC.Book.NextAskAbove(nil, 100))
    end)
  end)
