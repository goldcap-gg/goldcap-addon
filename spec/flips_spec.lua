...
describe("Data.Flips (Sniper v2 §D)", function()
  ...
  describe("GetFlips pruning", function()
    ...
    it("F1: no longer prunes a posted flip -- cost basis must survive while the lot is up", function()
      GC.Data.RecordFlip({ itemID = 1 }, purchase(), 0)
      GC.Data.MarkFlipPosted(1, 0)
      assert.equal(1, #GC.Data.GetFlips(0))
    end)

    it("F1: a posted flip still prunes once it crosses the AGE boundary (age is not reset by posting)", function()
      GC.Data.RecordFlip({ itemID = 1 }, purchase(), 0)
      GC.Data.MarkFlipPosted(1, 0)
      assert.equal(1, #GC.Data.GetFlips(FOURTEEN_DAYS))
      assert.equal(0, #GC.Data.GetFlips(FOURTEEN_DAYS + 1))
  ...
  describe("MarkFlipPosted / RemoveFlip", function()
    it("F1: marks the entry at the given index posted, WITHOUT removing it", function()
      GC.Data.RecordFlip({ itemID = 1 }, purchase(), 0)
      GC.Data.RecordFlip({ itemID = 2 }, purchase({ itemID = 2 }), 0)
      GC.Data.MarkFlipPosted(1)
      local flips = GC.Data.GetFlips(0)
      assert.equal(2, #flips)
      assert.is_true(flips[1].posted)
      assert.equal(1, flips[1].itemID)
      assert.equal(2, flips[2].itemID)
    end)

    it("F1: stamps postedAt with the injected `now`", function()
      GC.Data.RecordFlip({ itemID = 1 }, purchase(), 0)
      GC.Data.MarkFlipPosted(1, 555)
      assert.equal(555, GC.Data.GetFlips(0)[1].postedAt)
    end)

    it("F1: defaults postedAt to the current time when omitted", function()
      GC.Data.RecordFlip({ itemID = 1 }, purchase(), 0)
      local realTime = _G.time
      _G.time = function() return 777 end
      GC.Data.MarkFlipPosted(1)
      _G.time = realTime
      assert.equal(777, GC.Data.GetFlips(0)[1].postedAt)
    end)
