...
describe("Sell quote wiring", function()
  ...
  it("uses Latest for rendering and Fresh for every sell action, with no target fallback", function()
    local text = source()
    ...
    assert.is_truthy(render:find("GC.QuoteCache.Latest(quotes, f.itemID)", 1, true))
    assert.is_truthy(post:find("GC.QuoteCache.Fresh(quotes, flip.itemID, time())", 1, true))
    assert.is_truthy(repost:find("GC.QuoteCache.Fresh(quotes, flip.itemID, time())", 1, true))
